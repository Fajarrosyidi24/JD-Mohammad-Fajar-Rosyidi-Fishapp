      ...
      <div class="container d-flex align-items-center justify-content-between">
        ...
        <nav class="navmenu d-none d-lg-block">
          <ul class="list-unstyled m-0 d-flex gap-4">
            ...
            <li>
              <Link :href="route('product_auth', {role:role})" :class="{ active: routeCurrent('product_auth') }">Product</Link>
            </li>
          </ul>
        </nav>
      </div>
    ...
    <transition name="slide-fade">
      <nav v-if="isMenuOpen" class="mobile-nav d-lg-none">
        <ul class="list-unstyled m-0 p-0">
          <li>
            <Link :href="route('home', {role:role})" class="mobile-link" :class="{ active: routeCurrent('home') }" @click="toggleMenu">Beranda</Link>
          </li>
          <li>
            <Link :href="route('product_auth', {role:role})" class="mobile-link" :class="{ active: routeCurrent('product_auth') }"
              @click="toggleMenu">
            Product
            </Link>
          </li>
